% Preparation of Velocity Vector

pkg load statistics;

fid = fopen('ReadData-01639.txt','r');
...
tic
  for j = 1:1:N_max
    ...
    if(G_s(m) == 0)
      ...
      break
    elseif(G_s(m) < 0)
      A(j + 1) = min([max([A(j) (m - G_s(m)./(max(dG_s(A(j):(B(j)-A(j))/N_points:B(j)))) )])...
                      min([B(j) (m - G_s(m)./(min(dG_s(A(j):(B(j)-A(j))/N_points:B(j)))) )])]);
      B(j + 1) = max([max([A(j) (m - G_s(m)./(max(dG_s(A(j):(B(j)-A(j))/N_points:B(j)))) )])...
                      min([B(j) (m - G_s(m)./(min(dG_s(A(j):(B(j)-A(j))/N_points:B(j)))) )])]);
    elseif(G_s(m) > 0)
